...
describe Scanner do
  ...
  describe "#next_token" do
    it "recognizes identifiers" do
      ...
      %w[a bb c_c d1].each do |t|
        expect(s.next_token).to eq(Ast::Token.new(t, :id, 1))
      end
    end
    ...
    it "recognizes numerics" do
      s = Scanner.new("1 22")
      %w[1 22].each do |t|
        expect(s.next_token).to eq(Ast::Token.new(t, :num, 1))
      end
    end

    it "recognizes negative numerics" do
      s = Scanner.new("1 -22")
      expect(s.next_token).to eq(Ast::Token.new('1', :num, 1))
      expect(s.next_token).to eq(Ast::Token.new('-', :minus, 1))
      expect(s.next_token).to eq(Ast::Token.new('22', :num, 1))
    end

    it "splits number-identifier concatentations" do
      s = Scanner.new("1a")
      expect(s.next_token).to eq(Ast::Token.new('1', :num, 1))
      expect(s.next_token).to eq(Ast::Token.new('a', :id, 1))
    end

    it "recognizes strings" do
      s = Scanner.new('"string""another string"')
      expect(s.next_token).to eq(Ast::Token.new('string', :str, 1))
      expect(s.next_token).to eq(Ast::Token.new('another string', :str, 1))
    end
    ...
    it "recognizes single-character symbols" do
      s = Scanner.new(";,[]{}()+-*/%&")
      %w[; , [ ] { } ( ) + - * / % &].each do |t|
        expect(s.next_token).to eq(Ast::Token.new(t, Scanner::SYMBOLS[t], 1))
      end
    end
    ...
    it "returns nil on end-of-file" do
      s = Scanner.new("a")
      expect(s.next_token).to eq(Ast::Token.new('a', :id, 1))
      expect(s.next_token).to eq(Ast::Token.new(nil, :eof, 1))
    end
    ...
    it "consumes whitespace properly" do
      ...
      expect(s.next_token).to eq(Ast::Token.new('e', :id, 2))
    end
    ...
    it "bypasses multi-line comments and handles line-counting properly" do
      ...
      expect(s.next_token).to eq(Ast::Token.new('a', :id, 1))
      expect(s.next_token).to eq(Ast::Token.new('bb', :id, 2))
    end
  ...
  describe "#current_token" do
    it "returns the current token" do
      ...
      %w[a bb c_c d1].each do |t|
        s.next_token
        expect(s.current_token).to eq(Ast::Token.new(t, :id, 1))
      end
    end
